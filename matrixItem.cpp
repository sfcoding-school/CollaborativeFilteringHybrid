...
double calcoloSimilaritaItem(std::string rist_i, std::string rist_j, std::unordered_map<std::string, std::unordered_map<std::string, double> > & testSet){ 
    ...
    for(auto rist_i : listaRistoranti) {
      ...
      for(auto rist_j : listaRistoranti) {
        if (rist_j != "" && rist_i != rist_j)
        {
          ...
          if ( cosine_i_j != -99)
          {
              vettoreMieiSimili.insert({rist_j, cosine_i_j});
          }
        }
      }
